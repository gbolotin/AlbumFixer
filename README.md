...
- Classifies FLAC+CUE image splits, SACD/DSD extraction, existing-track metadata repair, and ambiguous sources. Verified end-to-end write-back is enabled for FLAC+CUE, one SACD ISO, and standalone existing FLAC tracks without a CUE sheet or source image; ambiguous modes stop before changing files.
...
- Repairs standalone existing FLAC tracks without requiring a CUE sheet. Existing nonempty tags and embedded artwork have highest priority; exact external catalog matches fill missing album fields and canonical track titles only when the remote tracklist agrees with existing-tag/filename hints, and filenames are the final title/number fallback. The app edits only local staging copies, proves every compressed FLAC audio-frame payload SHA-256 is unchanged, then replaces the originals through destination-side rollback. The **Delete originals** setting does not apply to this workflow.
